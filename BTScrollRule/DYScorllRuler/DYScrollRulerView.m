...
-(UITextField *)valueTF{
    if (!_valueTF) {
        _valueTF                          = [[UITextField alloc]initWithFrame:CGRectMake(self.bounds.size.width/2-60, 10,80, 60)];
        _valueTF.userInteractionEnabled   = YES;
        _valueTF.defaultTextAttributes    = @{NSFontAttributeName:[UIFont systemFontOfSize:24],NSForegroundColorAttributeName:[UIColor redColor]};
...
-(UILabel *)unitLab{
    if (!_unitLab) {
        _unitLab = [[UILabel alloc]initWithFrame:CGRectMake( CGRectGetMaxX(_valueTF.frame) + 10 , 10, 40, 40)];
    }
    return _unitLab;
}
